...
class GridWorld:
    def __init__(self, bounds, resolution):
        ...
        self.bounds = bounds
        self.res = resolution
        self.shape = np.ceil((bounds[1, :] - bounds[0, :] - 1e-6) / resolution).astype(
            np.int
        )
    ...
    def add_grid(
        self, name, init_value=0, extra_dims=(),
    ):
...
if __name__ == "__main__":
    h = GridWorld(np.array([[-0.25, -0.8], [0.25, -0.3]]), 0.005)
    ...
    cell_ids = h.pos_to_cell(np.array([[-0.25, -0.8], [-0.0, -0.5], [+0.25, -0.3],]))
