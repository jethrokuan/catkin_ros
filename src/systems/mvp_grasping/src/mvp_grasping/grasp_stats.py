...
def update_batch(data, ids, count, mean, var):
    ...
    ids_r_u, ids_r_u_i = np.unique(
        ids_r, return_inverse=True
    )  # TODO: This is by far the slowest part of this function.
    ids_r_u = (ids_r_u,)
    ids_r_u_i = (ids_r_u_i,)
    ...
    for d, m, v in zip(data, mean, var):
        ...
        new_mean = (count[ids_unr] * m[ids_unr] + count_temp * mean_temp) / (
            count[ids_unr] + count_temp
        )

        v[ids_unr] = (
            (count[ids_unr] * (v[ids_unr] + (m[ids_unr] - new_mean) ** 2))
            + count_temp * (var_temp + (mean_temp - new_mean) ** 2)
        ) / (count[ids_unr] + count_temp)
...
def update_histogram(data, ids, histogram):
    ids_r = (
        ids[:, 0] * histogram.shape[1]
        + ids[:, 1]
        + np.floor(data * 10.0).astype(np.int) * histogram.shape[0] * histogram.shape[1]
    )
    ids_r_u = np.unique(ids_r)
    ids_unr = (
        (ids_r_u[0] // histogram.shape[1]) % histogram.shape[0],
        ids_r_u[0] % histogram.shape[1],
        ids_r_u[0] // histogram.shape[0] // histogram.shape[1],
    )
