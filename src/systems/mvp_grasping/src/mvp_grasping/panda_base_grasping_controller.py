...
class Logger:
    ...
    def log_params(self, params):
        for p in params:
            self.log_list([p, rospy.get_param(p)])
    ...
    def log_dict(self, d, s=[]):
        if isinstance(d, dict):
            ...
            self.log_list(["/".join(s), d])


class Run:
    log_properties = ["success", "time", "viewpoints", "quality", "entropy"]
...
class Experiment:
    log_properties = ["success_rate", "mpph"]
    ...
    def mpph(self):
        return (
            3600 / (sum([r.time for r in self.runs]) / len(self.runs))
        ) * self.success_rate
...
class BaseGraspController(object):
    ...
    def __init__(self):
        ...
        self.pregrasp_pose = [
            (
                ...
                + rospy.get_param("/grasp_entropy_node/histogram/bounds/x1")
            )
            / 2,
            (
                rospy.get_param("/grasp_entropy_node/histogram/bounds/y2")
                + rospy.get_param("/grasp_entropy_node/histogram/bounds/y1")
            )
            / 2
            + 0.08,
    ...
    def __execute_best_grasp(self):
        ...
        self.best_grasp.pose.position.z = max(
            self.best_grasp.pose.position.z - 0.01, 0.026
        )  # 0.021 = collision with ground
